      ...
      <div className="max-w-6xl mx-auto px-4 flex items-center justify-between h-16">
        ...
        <Link href="#home" className="flex items-center gap-2">
          <div className="flex items-center gap-1.5">
            <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ backgroundColor: "#8B1A4A" }}>
            ...
            <div className="leading-tight">
              <div className="text-xs font-black tracking-widest uppercase" style={{ color: "#8B1A4A" }}>PREMIER</div>
              <div className="text-[9px] font-medium tracking-wider uppercase text-gray-500">SENIOR SOLUTIONS</div>
            </div>
          </div>
        </Link>
        ...
        <nav className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-sm font-medium text-gray-700 hover:text-[#8B1A4A] transition-colors"
            >
              {link.label}
            </Link>
          ))}
          <Link
            href="#contact"
            className="px-5 py-2 text-sm font-bold text-white rounded transition-colors"
        ...
        <div className="md:hidden bg-white border-t border-gray-100 px-4 py-4 flex flex-col gap-3">
          {navLinks.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              className="text-sm font-medium text-gray-700 hover:text-[#8B1A4A] py-1"
          ...
          <Link
            href="#contact"
            className="mt-2 px-5 py-2 text-sm font-bold text-white text-center rounded"
            style={{ backgroundColor: "#8B1A4A" }}
            onClick={() => setMobileOpen(false)}
          >
            CONTACT US
